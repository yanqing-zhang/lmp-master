...
class  User(Model):
    id = fields.BigIntField(pk=True, index=True, description="主键自增")
...
class UserRoleAssocations(Model):
    id = fields.BigIntField(pk=True, index=True, description="主键自增")
    user_id = fields.CharField(max_length=50, description="用户编号")
    role_id = fields.CharField(max_length=50, description="角色编号")
    create_user = fields.CharField(max_length=50, description="创建人编号")
    update_user = fields.CharField(max_length=50, description="修改人编号")
    create_at = fields.DatetimeField(description="创建时间", auto_now_add=True)
    update_at = fields.DatetimeField(description="修改时间", auto_now=True)
    yn = fields.IntField(description="此条数据是否可用,0为软删除，1正常", default=1)

    class Meta:
        table = "user_role_assocations"